...
class SettingCrudService extends BaseCrudService {
  ...
  async save2Excel(
    settings: any[],
    outputFullPath: string,
    templateFullPath: string = path.join(__dirname, 'templateSettings.xlsx'), // テンプレファイルは、指定されたファイルか、このソースがあるディレクトリ上のuntitled.xlsxを使う
    sheetName = 'Sheet1',
    applyStyles?: (instances: any[], workbook: any, sheetName: string) => void,
  ) {
    const applyStyles_ = applyStyles
    ...
    this.parent.util.save2Excel(settings, outputFullPath, templateFullPath, sheetName, applyStyles_)
  }
}
...
class UtilService {
  ...
  save2Excel(
    instances: any[],
    outputFullPath: string,
    templateFullPath: string = path.join(__dirname, 'templateUntitled.xlsx'), // テンプレファイルは、指定されたファイルか、このソースがあるディレクトリ上のtemplateUntitled使う
    sheetName = 'Sheet1',
    applyStyles?: (instances: any[], workbook: any, sheetName: string) => void,
  ): Promise<void> {
...
async function internalSave2Excel(
  instances: any[],
  outputFullPath: string,
  templateFullPath: string, // テンプレファイルは、指定されたファイルか、このソースがあるディレクトリ上のtemplateUntitled使う
  sheetName: string,
  applyStyles?: (instances: any[], workbook: any, sheetName: string) => void,
): Promise<void> {
